...
const FormattedMessage = ({ text, role }: { text: string; role: 'user' | 'model' | 'system' }) => {
  return (
    <div
      className={`max-w-[85%] p-3 rounded-2xl text-sm leading-relaxed shadow-lg overflow-hidden ${
        role === 'user'
  ...
  return (
    <div className="flex flex-col h-full w-full bg-slate-950/60 backdrop-blur-2xl border-r border-white/5 shadow-2xl relative overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-white/5 bg-slate-900/40 shrink-0">
        <Orb state={isLiveActive && !isLivePaused ? 'speaking' : (genieState.isThinking ? 'thinking' : 'neutral')} />
            ...
            Genie OS v2.0
             ...
             <div className="flex items-center gap-2 h-3 mt-0.5">
                ...
                {!isLiveActive && !genieState.isThinking && (
                     <span className="text-[9px] text-slate-500 font-medium tracking-wide">IDLE</span>
                )}
                 {!isLiveActive && genieState.isThinking && (
                     <span className="text-[9px] text-cyan-400 font-medium tracking-wide animate-pulse">THINKING</span>
                )}
             </div>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-white/5 bg-slate-900/30">
        ...
        {!isLiveActive && (
            <div className="mt-3 flex gap-2 overflow-x-auto no-scrollbar">
                <SuggestionChip text="How Neurons Work" onClick={() => onSendMessage("How Neurons Work")} />
                <SuggestionChip text="Python Basics" onClick={() => onSendMessage("Show me some Python code")} />
                <SuggestionChip text="Quiz Me" onClick={() => onSendMessage("Give me a quick quiz")} />
            </div>
        )}
      </div>
    </div>
