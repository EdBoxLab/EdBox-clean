...
export async function POST(request: NextRequest) {
  try {
    ...
    const contextSummaryArray = await extractContextFromText(extractedText);
    const contextSummary = contextSummaryArray.join('\n\n');
    ...
    if (isMessy(suggestedGoal) || suggestedGoal.length > 100) {
      ...
      suggestedGoal = suggestedGoal.split(' ').map((word: string) => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
    }
