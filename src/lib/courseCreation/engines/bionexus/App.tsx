...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-science-100 overflow-x-hidden">
      ...
      {/* Navigation Bar */}
      <nav className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm h-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between">
          <div className="flex items-center space-x-3 cursor-pointer" onClick={() => setActiveModule(null)}>
            <div className="bg-science-600 p-2 rounded-lg shadow-sm">
              <Microscope className="w-5 h-5 text-white" />
            </div>
            <span className="text-lg md:text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-science-700 to-science-500 hidden sm:block">
              BioLab Nexus
            </span>
             <span className="text-lg font-bold text-science-600 sm:hidden">BioLab</span>
          </div>
          
          {/* Desktop Nav Links */}
          <div className="hidden md:flex items-center space-x-6">
            <span className="text-xs font-medium px-2 py-1 rounded bg-slate-100 text-slate-500 border border-slate-200">v2.5.0</span>
            <div className="h-8 w-8 rounded-full bg-science-100 border border-science-200 flex items-center justify-center text-science-700 font-bold text-xs shadow-inner">
              JD
            </div>
          </div>
        ...
        {!activeModule ? (
          ...
          <div className="relative flex flex-col h-full overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-4 shrink-0">
              <div className="flex items-center space-x-4">
                ...
                >
                  ...
                  <span className="font-medium text-sm hidden sm:block">Back to Dashboard</span>
                </button>
                <div className="h-6 w-px bg-slate-200 hidden sm:block"></div>
                <h1 className="text-xl font-bold text-slate-800 truncate max-w-[200px] sm:max-w-none">{activeModule.name}</h1>
              ...
              <button 
                onClick={toggleAI}
                className={`flex items-center space-x-2 px-4 py-2 rounded-xl font-medium transition-all shadow-sm border ${
                   isAIOpen 
            ...
            <div className="flex-1 flex gap-6 overflow-hidden relative rounded-3xl">
              ...
              <div 
                className={`fixed inset-y-0 right-0 w-full sm:w-96 z-50 transform transition-transform duration-300 ease-out shadow-2xl ${
                    isAIOpen ? 'translate-x-0' : 'translate-x-full'
                } sm:relative sm:transform-none sm:w-80 sm:transition-all sm:duration-500 ${
                    isAIOpen ? 'sm:mr-0' : 'sm:-mr-[21rem] sm:opacity-0' // Negative margin to hide on desktop without unmounting
                }`}
              >
