...
import { ArcVisualizer } from "@/components/ui/ArcVisualizer";
...
// --- Helper Components ---
// Button, Card, ArcVisualizer imported from @/components/ui



// --- Main Application ---

const WriteLab = ({ challenge }: { challenge?: Challenge | null }) => {
  // State
  const [activeModule, setActiveModule] = useState(challenge?.context?.includes('academic') ? "academic" : "creative");
  const [activeTool, setActiveTool] = useState("analysis");
  const [content, setContent] = useState(challenge?.starterCode || "The neon lights flickered overhead, casting long, dancing shadows across the damp pavement. It was midnight in Sector 4, and the silence was heavy, broken only by the distant hum of the mag-lev trains.");
  const [notes, setNotes] = useState(challenge?.description ? `Goal: ${challenge.title}\n\nUser Notes:` : "");

  useEffect(() => {
    if (challenge) {
      if (challenge.starterCode) setContent(challenge.starterCode);
      if (challenge.title) setNotes(prev => `Task: ${challenge.title}\n${challenge.description}\n\n` + prev);
    }
  }, [challenge]);
  ...
  const SidebarItem = ({ item, collapsed }: any) => (
    <button
      ...
      className={`w-full flex items-center gap-3 p-3 rounded-lg transition-colors ${activeModule === item.id
  ...
  return (
    <div className="flex flex-col h-full w-full overflow-hidden bg-[#28282d] text-zinc-100 rounded-xl">

      {/* Toolbar */}
      <header className="h-16 bg-[#28282d] border-b border-zinc-700/50 flex items-center justify-between px-6 shrink-0 z-20 gap-4">
        <div className="flex items-center gap-4 overflow-x-auto no-scrollbar">
          <div className="flex items-center gap-2 shrink-0">
            ...
            <span className="font-semibold tracking-tight hidden sm:inline">WriteLab</span>
          </div>
          <div className="h-6 w-px bg-zinc-700 shrink-0 mx-2"></div>
          {/* Module Selector */}
          <div className="flex items-center gap-1">
            {MODULES.map(m => (
              <button
                ...
                className={`p-2 rounded-lg transition-colors ${activeModule === m.id ? 'bg-indigo-600/20 text-indigo-300' : 'text-zinc-400 hover:text-zinc-100'}`}
                title={m.name}
              >
                <m.icon size={18} />
              </button>
          ...
          <span className="px-2 py-0.5 bg-zinc-800 text-zinc-400 text-xs rounded-full border border-zinc-700 shrink-0">Draft</span>
        </div>
          ...
          <div className="flex border-b border-zinc-800">
            {TOOLS.map(tool => (
              <button
                ...
                className={`flex-1 p-3 flex justify-center items-center gap-2 text-sm font-medium transition-colors border-b-2 ${activeTool === tool.id
