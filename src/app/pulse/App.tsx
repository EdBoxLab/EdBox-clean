...
const App: React.FC = () => {
  ...
  const [genieState, setGenieState] = useState<GenieState>({ isThinking: false, mood: 'neutral' });
  ...
  const handleSendMessage = async (text: string) => {
    ...
    setGenieState({ isThinking: true, mood: 'thinking' });
    ...
    try {
      ...
      setGenieState({ isThinking: false, mood: 'neutral' });
    }
  };
  ...
  const toggleLiveMode = async () => {
      if (isLiveActive) {
          ...
          setIsLivePaused(false);
          setGenieState({ isThinking: false, mood: 'listening' });
            ...
            <div className="flex items-center gap-3">
                {windows.map((win) => (
                    <motion.div 
                        ...
                        className={`group relative flex flex-col items-center justify-center w-12 h-12 rounded-xl border transition-all cursor-pointer flex-shrink-0 ${
                            !win.isMinimized 
