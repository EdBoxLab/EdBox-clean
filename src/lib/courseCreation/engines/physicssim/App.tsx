...
export const App: React.FC = () => {
  ...
  return (
    <div className="flex h-screen w-screen bg-slate-950 text-slate-200 font-sans overflow-hidden selection:bg-blue-500/30">
      <Sidebar activeModuleId={activeModuleId} onSelectModule={setActiveModuleId} collapsed={viewMode === 'result'} />
      <div className="flex-1 flex flex-col relative min-w-0 bg-slate-950">
        <header className="h-20 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-8 shrink-0 z-20">
          <div className="flex items-center">
            {viewMode === 'result' && (
              <button onClick={() => { setStatus(SimulationStatus.PAUSED); setViewMode('setup'); }} className="mr-4 p-2 rounded-full hover:bg-slate-800 text-slate-400 hover:text-white transition-colors">
                <ArrowLeft size={20} />
              </button>
            )}
            <div>
          ...
          <div className="flex bg-slate-950 p-1.5 rounded-lg border border-slate-800">
            <button onClick={() => { setStatus(SimulationStatus.PAUSED); setViewMode('setup'); }} className={`px-5 py-2 rounded-md text-sm font-bold transition-all flex items-center ${viewMode === 'setup' ? 'bg-slate-800 text-white shadow-sm ring-1 ring-slate-700' : 'text-slate-500 hover:text-slate-300'}`}>
              <Settings size={16} className="mr-2" /> Setup
            </button>
            <button onClick={() => setViewMode('result')} className={`px-5 py-2 rounded-md text-sm font-bold transition-all flex items-center ${viewMode === 'result' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}>
