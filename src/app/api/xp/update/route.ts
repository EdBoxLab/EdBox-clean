...
export async function POST(request: NextRequest) {
  try {
    ...
    const learnerStateId = skillGraphId || 'default';
    ...
    return NextResponse.json({
      success: true,
      xp: { gained: xpGained, total: newTotalXp, level: newLevel, leveledUp },
      streak: newStreak,
      activity
    });
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: error.message || 'Failed to fetch XP' }, { status: 500 });
  }
}
