...
const LeaderboardRow = ({ entry }: { entry: any }) => (
  <div className={`flex items-center p-3 rounded-lg transition-all border ${entry.isUser ? 'border-indigo-500 bg-indigo-600/10' : 'border-zinc-800'}`}>
...
const CircleLeaderboardRow = ({ entry }: { entry: any }) => (
  <div className="flex items-center p-3 rounded-lg border border-zinc-800">
    <span className={`font-bold text-lg w-10 ${entry.rank <= 3 ? 'text-yellow-400' : 'text-gray-400'}`}>{entry.rank}</span>
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="border border-zinc-800 rounded-2xl p-6 mb-12">
