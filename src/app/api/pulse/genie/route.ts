...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!message) {
      log('PULSE-GENIE', 'Error: No message provided');
    ...
    log('PULSE-GENIE', 'Groq keys available', { keyCount: groqKeys.length });

    if (!hasGroqKey || demoMode) {
      log('PULSE-GENIE', 'Running in demo mode', { hasGroqKey, demoMode });
    ...
    log('PULSE-GENIE', 'Message added to context', {
    ...
    log('PULSE-GENIE', 'Using Groq API key', { keyIndex: keyIndex >= 0 ? keyIndex + 1 : 'unknown' });

    const Groq = (await import('groq-sdk')).default;
    const groq = new Groq({ apiKey });

    log('PULSE-GENIE', 'Calling Groq API', { model: 'llama-3.3-70b-versatile' });
    ...
    if (!assistantMessage) {
      log('PULSE-GENIE', 'Error: No response from Groq');
      messages.pop();
      return NextResponse.json({
    ...
    if (assistantMessage.tool_calls && assistantMessage.tool_calls.length > 0) {
      log('PULSE-GENIE', 'Tool calls received', { 
      ...
      log('PULSE-GENIE', 'Making follow-up Groq call after tool execution');
    ...
    const duration = Date.now() - startTime;
    log('PULSE-GENIE', 'Error occurred', {
      error: error.message,
      stack: error.stack,
      duration: `${duration}ms`
    });
...
export async function DELETE(request: NextRequest) {
  ...
  log('PULSE-GENIE', 'Session clear requested', { sessionId });
