    ...
    const handleChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
        ...
        debounceRef.current = setTimeout(() => {
            ...
            widgetTelemetry.fire({
                ...
                widget_type: 'NOTE_WRITER',
                event_data: { word_count: newVal.split(/\s+/).filter(w => w.length > 0).length }
            });
        }, 1500);
    };
    ...
    return (
        <div className="flex flex-col h-full bg-[#fffbeb] text-slate-800 font-serif relative">
            <div className="flex items-center space-x-1 p-2 bg-[#f3e9d2] border-b border-[#e6dcc0] shrink-0">
                ...
                <button onClick={() => { setViewMode('preview'); widgetTelemetry.fire({ event_type: 'note_previewed', widget_type: 'NOTE_WRITER' }); }} className={`p-1.5 rounded transition-colors ${viewMode === 'preview' ? 'bg-black/10 text-slate-900' : 'text-slate-500 hover:text-slate-700'}`} title="Preview Mode (Markdown/LaTeX)">
                    <Eye size={16} />
                </button>
